...
If we reached None node, then we just return, we are out of tree. If current node do not have children, then we check that it is left children of some 
other node and if it is the case, we add its value to global self.sum.
